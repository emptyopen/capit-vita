...
class Robinhood:
    ...
    def securities_owned(self):
        ...
        for position in positions['results']:
            ...
            if quantity > 0:
                stock_name = self.session.get(position['instrument']).json()['symbol']
                average_buy_price = float(position['average_buy_price'])
                date_bought = position['created_at']
                current_price = float(self.last_trade_price(stock_name))
                securities.append([stock_name, quantity, date_bought, average_buy_price, current_price])
        return securities
